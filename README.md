<p align="center">
  <img src="./assets/animated-bg.svg" alt="animated background" width="100%" />
</p>

# <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=F75C7E&center=true&vCenter=true&width=435&lines=Welcome+to+My+Profile!;Full+Stack+Developer;Open+Source+Enthusiast;Lifelong+Learner" alt="Typing SVG" />

<p align="center">
  <img src="https://spotify-github-profile.vercel.app/api/view?uid=YOURSPOTIFYID&cover_image=true&theme=novatorem&bar_color=53b14f&bar_color_cover=false" alt="Spotify Now Playing" width="350"/>
  <img src="https://lanyard.cnrad.dev/api/YOURDISCORDID" alt="Discord Status" width="350"/>
</p>

---

## 🛠️ 技術スタック

### フロントエンド
<p>
  <img src="https://skillicons.dev/icons?i=vue,js,html" />
</p>

### バックエンド
<p>
  <img src="https://skillicons.dev/icons?i=js,python,ruby,flask,fastapi" />
</p>

### データベース
<p>
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=database&logoColor=white"/>
</p>

### クラウド・DevOps
<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,githubactions" />
</p>

### ツール
<p>
  <img src="https://skillicons.dev/icons?i=github" />
  <img src="https://img.shields.io/badge/RubyMine-FA2C2C?style=for-the-badge&logo=rubymine&logoColor=white"/>
</p>

---

## 📊 GitHub統計
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=${{ github.repository_owner }}&show_icons=true&theme=radical" height="180"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=${{ github.repository_owner }}&layout=compact&theme=radical" height="180"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=${{ github.repository_owner }}&theme=radical" height="180"/>
  <br/>
  <img src="https://github-profile-trophy.vercel.app/?username=${{ github.repository_owner }}&theme=dracula&margin-w=15&no-frame=true"/>
  <br/>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=${{ github.repository_owner }}&theme=dracula"/>
  <br/>
  <img src="https://raw.githubusercontent.com/${{ github.repository_owner }}/${{ github.repository_owner }}/output/github-snake.svg" alt="Snake animation"/>
  <br/>
  <img src="https://raw.githubusercontent.com/${{ github.repository_owner }}/${{ github.repository_owner }}/output/github-snake-dark.svg?palette=github-dark" alt="Snake animation dark"/>
  <br/>
  <img src="https://github.com/${{ github.repository_owner }}/${{ github.repository_owner }}/blob/main/profile-3d-contrib/profile-night-rainbow.svg" alt="3D Contribution"/>
</p>

---

## 👀 プロフィール閲覧数
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=${{ github.repository_owner }}&style=for-the-badge"/>
</p>

---

## 🚀 セットアップ手順・Secrets設定

- 必要なAPIキーは`GITHUB_TOKEN`（GitHub Actions用トークン）のみです。
- `.github/workflows/`配下のYAMLファイルやREADME.mdは、ユーザー名や個別IDの手動入力なしで動作します。
- 必要に応じてassetsディレクトリに画像を追加してください。

---

> **注意:** 本READMEはGitHubプロフィール用に最適化されています。各種APIや外部サービスの仕様変更により一部表示が崩れる場合があります。
